...
export JOIN_TOKEN_A=$(sudo spire-server token generate -spiffeID spiffe://example.org/agentA -socketPath /var/run/spire/server/sockets/a/private/api.sock | awk '{print "\""$2"\""}')
export JOIN_TOKEN_B=$(sudo spire-server token generate -spiffeID spiffe://example.org/agentB -socketPath /var/run/spire/server/sockets/b/private/api.sock | awk '{print "\""$2"\""}')
